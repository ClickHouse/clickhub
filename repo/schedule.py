...
def is_job_scheduled(client: RepoClickHouseClient, task_table, repo_name):
    response = client.query_row(
        f"SELECT * FROM {task_table} WHERE repo_name='{repo_name}'"
    )
...
def schedule_repo_job(
    client: RepoClickHouseClient,
    task_table: str,
    repo_name: str,
    priority: int,
    max_queue_length=sys.maxsize,
):
    if queue_length(client, task_table) > max_queue_length:
        raise FullQueue(
            f"cannot schedule [{repo_name}]. Max queue size [{max_queue_length}] exceeded."
        )
    ...
    try:
        client.insert_row(
            task_table,
            ["repo_name", "scheduled", "priority", "worker_id", "started_time"],
...
def schedule_all_current_repos(
    client: RepoClickHouseClient,
    repo_table,
    task_table,
    priority,
    ...
    repo_name_batches = [
        repo_names[i : i + batch_size] for i in range(0, len(repo_names), batch_size)
    ]
    for repo_batch in repo_name_batches:
        ...
        logging.info(
            f"repos currently scheduled, will be ignored: [{currently_scheduled}]"
        )
