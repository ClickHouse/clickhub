...
class TestApp(unittest.TestCase):
    ...
    def test_repos_in_db(self):
        ...
        responce = requests.get('http://localhost:5000/add_new_repo?repo=ClickHouse/opentelemetry-demo')
        self.assertEqual(200, responce.status_code)
    
    def test_repos_in_queue(self):
        res = client.query_row("INSERT INTO git.new_queue (repo_name) VALUES ('ClickHouse/opentelemetry-demo')")
        responce = requests.get('http://localhost:5000/add_new_repo?repo=ClickHouse/opentelemetry-demo')
        res = client.query_row("DELETE FROM git.new_queue WHERE repo_name='ClickHouse/opentelemetry-demo'")
        self.assertEqual(200, responce.status_code)
